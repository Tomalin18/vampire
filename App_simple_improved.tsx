...
export default function App() {
  ...
  }, [enemies.length, player.x, player.y]);
  ...
  const updateGame = useCallback((deltaTime: number) => {
    // 更新玩家在虛擬世界中的位置（保持虛擬座標系統）
    setPlayer(prev => {
      const newX = prev.x + prev.velocityX * deltaTime;
      const newY = prev.y + prev.velocityY * deltaTime;
  ...
  }, [enemies, player.x, player.y, player.velocityX, player.velocityY]);
  ...
  const renderMenuScreen = () => (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.menuContainer}>
        ...
        <Text style={styles.subtitle}>簡化改進版</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.gameHUD}>
       ...
       <View style={styles.gameArea} {...panResponder.panHandlers}>
         ...
         {enemies.map(enemy => {
           // 計算敵人相對於玩家的螢幕位置
           const relativeX = enemy.x - player.x + SCREEN_WIDTH / 2;
           const relativeY = enemy.y - player.y + SCREEN_HEIGHT / 2;
